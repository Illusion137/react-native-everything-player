...
`TrackPlayer` now exposes SABR-oriented bridge methods for upcoming native integration:
- `downloadSabr(params, outputPath)`
- `updateSabrStream(outputPath, sabrServerUrl, sabrUstreamerConfig)`
- `updateSabrPoToken(outputPath, poToken)`
- `onSabrDownloadProgress(...)`
- `onSabrReloadPlayerResponse(...)`
- `onSabrRefreshPoToken(...)`

`downloadSabrManaged(...)` is also exported as a JS helper that wires progress + token/reload callback flows around `TrackPlayer.downloadSabr`.

Use `npm run sabr:protos` after cloning `googlevideo` into `vendor/googlevideo` to generate C++ protobuf headers for SABR work.

Widevine status is exposed via `TrackPlayer.isWidevineSupported()` (currently `true` on Android, `false` on iOS).

For source selection and cache-aware URL resolution, use `selectPreferredSource(...)` and `resolvePlaybackUrl(...)`.
