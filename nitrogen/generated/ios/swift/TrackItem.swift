///
/// TrackItem.swift
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

import NitroModules

/**
 * Represents an instance of `TrackItem`, backed by a C++ struct.
 */
public typealias TrackItem = margelo.nitro.nitroplayer.TrackItem

public extension TrackItem {
  private typealias bridge = margelo.nitro.nitroplayer.bridge.swift

  /**
   * Create a new instance of `TrackItem`.
   */
  init(id: String, title: String, artist: String, album: String, duration: Double, url: String, artwork: Variant_NullType_String?, drm: Variant_NullType_TrackDrmConfig?, extraPayload: AnyMap?) {
    self.init(std.string(id), std.string(title), std.string(artist), std.string(album), duration, std.string(url), { () -> bridge.std__optional_std__variant_nitro__NullType__std__string__ in
      if let __unwrappedValue = artwork {
        return bridge.create_std__optional_std__variant_nitro__NullType__std__string__({ () -> bridge.std__variant_nitro__NullType__std__string_ in
          switch __unwrappedValue {
            case .first(let __value):
              return bridge.create_std__variant_nitro__NullType__std__string_(margelo.nitro.NullType.null)
            case .second(let __value):
              return bridge.create_std__variant_nitro__NullType__std__string_(std.string(__value))
          }
        }().variant)
      } else {
        return .init()
      }
    }(), { () -> bridge.std__optional_std__variant_nitro__NullType__TrackDrmConfig__ in
      if let __unwrappedValue = drm {
        return bridge.create_std__optional_std__variant_nitro__NullType__TrackDrmConfig__({ () -> bridge.std__variant_nitro__NullType__TrackDrmConfig_ in
          switch __unwrappedValue {
            case .first(let __value):
              return bridge.create_std__variant_nitro__NullType__TrackDrmConfig_(margelo.nitro.NullType.null)
            case .second(let __value):
              return bridge.create_std__variant_nitro__NullType__TrackDrmConfig_(__value)
          }
        }().variant)
      } else {
        return .init()
      }
    }(), { () -> bridge.std__optional_std__shared_ptr_AnyMap__ in
      if let __unwrappedValue = extraPayload {
        return bridge.create_std__optional_std__shared_ptr_AnyMap__(__unwrappedValue.cppPart)
      } else {
        return .init()
      }
    }())
  }

  @inline(__always)
  var id: String {
    return String(self.__id)
  }
  
  @inline(__always)
  var title: String {
    return String(self.__title)
  }
  
  @inline(__always)
  var artist: String {
    return String(self.__artist)
  }
  
  @inline(__always)
  var album: String {
    return String(self.__album)
  }
  
  @inline(__always)
  var duration: Double {
    return self.__duration
  }
  
  @inline(__always)
  var url: String {
    return String(self.__url)
  }
  
  @inline(__always)
  var artwork: Variant_NullType_String? {
    return { () -> Variant_NullType_String? in
      if bridge.has_value_std__optional_std__variant_nitro__NullType__std__string__(self.__artwork) {
        let __unwrapped = bridge.get_std__optional_std__variant_nitro__NullType__std__string__(self.__artwork)
        return { () -> Variant_NullType_String in
          let __variant = bridge.std__variant_nitro__NullType__std__string_(__unwrapped)
          switch __variant.index() {
            case 0:
              let __actual = __variant.get_0()
              return .first(NullType.null)
            case 1:
              let __actual = __variant.get_1()
              return .second(String(__actual))
            default:
              fatalError("Variant can never have index \(__variant.index())!")
          }
        }()
      } else {
        return nil
      }
    }()
  }
  
  @inline(__always)
  var drm: Variant_NullType_TrackDrmConfig? {
    return { () -> Variant_NullType_TrackDrmConfig? in
      if bridge.has_value_std__optional_std__variant_nitro__NullType__TrackDrmConfig__(self.__drm) {
        let __unwrapped = bridge.get_std__optional_std__variant_nitro__NullType__TrackDrmConfig__(self.__drm)
        return { () -> Variant_NullType_TrackDrmConfig in
          let __variant = bridge.std__variant_nitro__NullType__TrackDrmConfig_(__unwrapped)
          switch __variant.index() {
            case 0:
              let __actual = __variant.get_0()
              return .first(NullType.null)
            case 1:
              let __actual = __variant.get_1()
              return .second(__actual)
            default:
              fatalError("Variant can never have index \(__variant.index())!")
          }
        }()
      } else {
        return nil
      }
    }()
  }
  
  @inline(__always)
  var extraPayload: AnyMap? {
    return { () -> AnyMap? in
      if bridge.has_value_std__optional_std__shared_ptr_AnyMap__(self.__extraPayload) {
        let __unwrapped = bridge.get_std__optional_std__shared_ptr_AnyMap__(self.__extraPayload)
        return AnyMap(withCppPart: __unwrapped)
      } else {
        return nil
      }
    }()
  }
}
