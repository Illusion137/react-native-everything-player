///
/// HybridTrackPlayerSpec.kt
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

package com.margelo.nitro.nitroplayer

import androidx.annotation.Keep
import com.facebook.jni.HybridData
import com.facebook.proguard.annotations.DoNotStrip
import com.margelo.nitro.core.Promise
import com.margelo.nitro.core.HybridObject

/**
 * A Kotlin class representing the TrackPlayer HybridObject.
 * Implement this abstract class to create Kotlin-based instances of TrackPlayer.
 */
@DoNotStrip
@Keep
@Suppress(
  "KotlinJniMissingFunction", "unused",
  "RedundantSuppression", "RedundantUnitReturnType", "SimpleRedundantLet",
  "LocalVariableName", "PropertyName", "PrivatePropertyName", "FunctionName"
)
abstract class HybridTrackPlayerSpec: HybridObject() {
  // Properties
  

  // Methods
  @DoNotStrip
  @Keep
  abstract fun play(): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun pause(): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun playSong(songId: String, fromPlaylist: String?): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun skipToNext(): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun skipToIndex(index: Double): Promise<Boolean>
  
  @DoNotStrip
  @Keep
  abstract fun skipToPrevious(): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun seek(position: Double): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun addToUpNext(trackId: String): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun playNext(trackId: String): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun getActualQueue(): Promise<Array<TrackItem>>
  
  @DoNotStrip
  @Keep
  abstract fun getState(): Promise<PlayerState>
  
  @DoNotStrip
  @Keep
  abstract fun setRepeatMode(mode: RepeatMode): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun getRepeatMode(): RepeatMode
  
  @DoNotStrip
  @Keep
  abstract fun configure(config: PlayerConfig): Promise<Unit>
  
  abstract fun onChangeTrack(callback: (track: TrackItem, reason: Reason?) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onChangeTrack_cxx(callback: Func_void_TrackItem_std__optional_Reason_): Unit {
    val __result = onChangeTrack(callback)
    return __result
  }
  
  abstract fun onPlaybackStateChange(callback: (state: TrackPlayerState, reason: Reason?) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onPlaybackStateChange_cxx(callback: Func_void_TrackPlayerState_std__optional_Reason_): Unit {
    val __result = onPlaybackStateChange(callback)
    return __result
  }
  
  abstract fun onSeek(callback: (position: Double, totalDuration: Double) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onSeek_cxx(callback: Func_void_double_double): Unit {
    val __result = onSeek(callback)
    return __result
  }
  
  abstract fun onPlaybackProgressChange(callback: (position: Double, totalDuration: Double, isManuallySeeked: Boolean?) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onPlaybackProgressChange_cxx(callback: Func_void_double_double_std__optional_bool_): Unit {
    val __result = onPlaybackProgressChange(callback)
    return __result
  }
  
  abstract fun onAndroidAutoConnectionChange(callback: (connected: Boolean) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onAndroidAutoConnectionChange_cxx(callback: Func_void_bool): Unit {
    val __result = onAndroidAutoConnectionChange(callback)
    return __result
  }
  
  @DoNotStrip
  @Keep
  abstract fun isAndroidAutoConnected(): Boolean
  
  @DoNotStrip
  @Keep
  abstract fun setVolume(volume: Double): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun updateTracks(tracks: Array<TrackItem>): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun getTracksById(trackIds: Array<String>): Promise<Array<TrackItem>>
  
  @DoNotStrip
  @Keep
  abstract fun getTracksNeedingUrls(): Promise<Array<TrackItem>>
  
  @DoNotStrip
  @Keep
  abstract fun getNextTracks(count: Double): Promise<Array<TrackItem>>
  
  @DoNotStrip
  @Keep
  abstract fun getCurrentTrackIndex(): Promise<Double>
  
  abstract fun onTracksNeedUpdate(callback: (tracks: Array<TrackItem>, lookahead: Double) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onTracksNeedUpdate_cxx(callback: Func_void_std__vector_TrackItem__double): Unit {
    val __result = onTracksNeedUpdate(callback)
    return __result
  }
  
  @DoNotStrip
  @Keep
  abstract fun setPlaybackSpeed(speed: Double): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun getPlaybackSpeed(): Promise<Double>
  
  @DoNotStrip
  @Keep
  abstract fun removeFromPlayNext(trackId: String): Promise<Boolean>
  
  @DoNotStrip
  @Keep
  abstract fun removeFromUpNext(trackId: String): Promise<Boolean>
  
  @DoNotStrip
  @Keep
  abstract fun clearPlayNext(): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun clearUpNext(): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun reorderTemporaryTrack(trackId: String, newIndex: Double): Promise<Boolean>
  
  @DoNotStrip
  @Keep
  abstract fun getPlayNextQueue(): Promise<Array<TrackItem>>
  
  @DoNotStrip
  @Keep
  abstract fun getUpNextQueue(): Promise<Array<TrackItem>>
  
  abstract fun onTemporaryQueueChange(callback: (playNextQueue: Array<TrackItem>, upNextQueue: Array<TrackItem>) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onTemporaryQueueChange_cxx(callback: Func_void_std__vector_TrackItem__std__vector_TrackItem_): Unit {
    val __result = onTemporaryQueueChange(callback)
    return __result
  }
  
  @DoNotStrip
  @Keep
  abstract fun downloadSabr(params: SabrDownloadParams, outputPath: String): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun updateSabrStream(outputPath: String, sabrServerUrl: String, sabrUstreamerConfig: String): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun updateSabrPoToken(outputPath: String, poToken: String): Promise<Unit>
  
  abstract fun onSabrDownloadProgress(callback: (progress: SabrDownloadProgress) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onSabrDownloadProgress_cxx(callback: Func_void_SabrDownloadProgress): Unit {
    val __result = onSabrDownloadProgress(callback)
    return __result
  }
  
  abstract fun onSabrReloadPlayerResponse(callback: (request: SabrReloadPlayerResponseRequest) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onSabrReloadPlayerResponse_cxx(callback: Func_void_SabrReloadPlayerResponseRequest): Unit {
    val __result = onSabrReloadPlayerResponse(callback)
    return __result
  }
  
  abstract fun onSabrRefreshPoToken(callback: (request: SabrRefreshPoTokenRequest) -> Unit): Unit
  
  @DoNotStrip
  @Keep
  private fun onSabrRefreshPoToken_cxx(callback: Func_void_SabrRefreshPoTokenRequest): Unit {
    val __result = onSabrRefreshPoToken(callback)
    return __result
  }
  
  @DoNotStrip
  @Keep
  abstract fun isWidevineSupported(): Boolean
  
  @DoNotStrip
  @Keep
  abstract fun setCrossfadeDuration(seconds: Double): Promise<Unit>
  
  @DoNotStrip
  @Keep
  abstract fun getCrossfadeDuration(): Promise<Double>

  // Default implementation of `HybridObject.toString()`
  override fun toString(): String {
    return "[HybridObject TrackPlayer]"
  }

  // C++ backing class
  @DoNotStrip
  @Keep
  protected open class CxxPart(javaPart: HybridTrackPlayerSpec): HybridObject.CxxPart(javaPart) {
    // C++ JHybridTrackPlayerSpec::CxxPart::initHybrid(...)
    external override fun initHybrid(): HybridData
  }
  override fun createCxxPart(): CxxPart {
    return CxxPart(this)
  }

  companion object {
    protected const val TAG = "HybridTrackPlayerSpec"
  }
}
