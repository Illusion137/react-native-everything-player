///
/// JTrackItem.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

#pragma once

#include <fbjni/fbjni.h>
#include "TrackItem.hpp"

#include "JTrackDrmConfig.hpp"
#include "JVariant_NullType_String.hpp"
#include "JVariant_NullType_TrackDrmConfig.hpp"
#include "TrackDrmConfig.hpp"
#include <NitroModules/AnyMap.hpp>
#include <NitroModules/JAnyMap.hpp>
#include <NitroModules/JNull.hpp>
#include <NitroModules/Null.hpp>
#include <optional>
#include <string>
#include <variant>

namespace margelo::nitro::nitroplayer {

  using namespace facebook;

  /**
   * The C++ JNI bridge between the C++ struct "TrackItem" and the the Kotlin data class "TrackItem".
   */
  struct JTrackItem final: public jni::JavaClass<JTrackItem> {
  public:
    static constexpr auto kJavaDescriptor = "Lcom/margelo/nitro/nitroplayer/TrackItem;";

  public:
    /**
     * Convert this Java/Kotlin-based struct to the C++ struct TrackItem by copying all values to C++.
     */
    [[maybe_unused]]
    [[nodiscard]]
    TrackItem toCpp() const {
      static const auto clazz = javaClassStatic();
      static const auto fieldId = clazz->getField<jni::JString>("id");
      jni::local_ref<jni::JString> id = this->getFieldValue(fieldId);
      static const auto fieldTitle = clazz->getField<jni::JString>("title");
      jni::local_ref<jni::JString> title = this->getFieldValue(fieldTitle);
      static const auto fieldArtist = clazz->getField<jni::JString>("artist");
      jni::local_ref<jni::JString> artist = this->getFieldValue(fieldArtist);
      static const auto fieldAlbum = clazz->getField<jni::JString>("album");
      jni::local_ref<jni::JString> album = this->getFieldValue(fieldAlbum);
      static const auto fieldDuration = clazz->getField<double>("duration");
      double duration = this->getFieldValue(fieldDuration);
      static const auto fieldUrl = clazz->getField<jni::JString>("url");
      jni::local_ref<jni::JString> url = this->getFieldValue(fieldUrl);
      static const auto fieldArtwork = clazz->getField<JVariant_NullType_String>("artwork");
      jni::local_ref<JVariant_NullType_String> artwork = this->getFieldValue(fieldArtwork);
      static const auto fieldDrm = clazz->getField<JVariant_NullType_TrackDrmConfig>("drm");
      jni::local_ref<JVariant_NullType_TrackDrmConfig> drm = this->getFieldValue(fieldDrm);
      static const auto fieldExtraPayload = clazz->getField<JAnyMap::javaobject>("extraPayload");
      jni::local_ref<JAnyMap::javaobject> extraPayload = this->getFieldValue(fieldExtraPayload);
      return TrackItem(
        id->toStdString(),
        title->toStdString(),
        artist->toStdString(),
        album->toStdString(),
        duration,
        url->toStdString(),
        artwork != nullptr ? std::make_optional(artwork->toCpp()) : std::nullopt,
        drm != nullptr ? std::make_optional(drm->toCpp()) : std::nullopt,
        extraPayload != nullptr ? std::make_optional(extraPayload->cthis()->getMap()) : std::nullopt
      );
    }

  public:
    /**
     * Create a Java/Kotlin-based struct by copying all values from the given C++ struct to Java.
     */
    [[maybe_unused]]
    static jni::local_ref<JTrackItem::javaobject> fromCpp(const TrackItem& value) {
      using JSignature = JTrackItem(jni::alias_ref<jni::JString>, jni::alias_ref<jni::JString>, jni::alias_ref<jni::JString>, jni::alias_ref<jni::JString>, double, jni::alias_ref<jni::JString>, jni::alias_ref<JVariant_NullType_String>, jni::alias_ref<JVariant_NullType_TrackDrmConfig>, jni::alias_ref<JAnyMap::javaobject>);
      static const auto clazz = javaClassStatic();
      static const auto create = clazz->getStaticMethod<JSignature>("fromCpp");
      return create(
        clazz,
        jni::make_jstring(value.id),
        jni::make_jstring(value.title),
        jni::make_jstring(value.artist),
        jni::make_jstring(value.album),
        value.duration,
        jni::make_jstring(value.url),
        value.artwork.has_value() ? JVariant_NullType_String::fromCpp(value.artwork.value()) : nullptr,
        value.drm.has_value() ? JVariant_NullType_TrackDrmConfig::fromCpp(value.drm.value()) : nullptr,
        value.extraPayload.has_value() ? JAnyMap::create(value.extraPayload.value()) : nullptr
      );
    }
  };

} // namespace margelo::nitro::nitroplayer
